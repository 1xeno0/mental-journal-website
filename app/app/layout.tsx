...
export default function ProtectedLayout({
  ...
  if (!authorized) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-sky-400 via-sky-200 to-slate-50">
        <div className="animate-pulse flex flex-col items-center gap-4">
          <div className="h-12 w-12 rounded-full bg-white/20 backdrop-blur-sm"></div>
          <div className="h-4 w-32 rounded bg-white/20 backdrop-blur-sm"></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-b from-sky-400 via-sky-200 to-slate-50">
      ...
      <main className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-8 space-y-12">
        {children}
      </main>
    </div>
  );
